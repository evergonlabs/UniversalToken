  ...
  ERC1820Implementer
{
  ...
  function _getNextStartTime(
    ...
    if (previousStartTime >= block.timestamp) {
      ...
      return
        block.timestamp.sub(
          (block.timestamp - previousStartTime).mod(subscriptionPeriod)
        );
    }
  }
  ...
  function _subscribe(
    ...
    if (
      cycle.assetValueType == AssetValue.Known && executePaymentAtSubscription
    ) {
  ...
  function cancelOrder(uint256 orderIndex) external {
    ...
    require(
      order.state == OrderState.Subscribed || order.state == OrderState.Paid,
  ...
  function valuate(
    ...
    require(
      cycleState > CycleState.Subscription && cycleState < CycleState.Payment,
   ...
   */
  function executePaymentAsController(
    uint256 orderIndex,
    bool bypassPayment
  ) external {
    Order storage order = _orders[orderIndex];
  ...
  function _executePayment(
    ...
    require(
      order.orderType == OrderType.Value || order.orderType == OrderType.Amount,
      "Invalid order type"
    );
    ...
    if (!bypassPayment) {
      if (cycle.paymentType == Payment.ETH) {
        ...
        cycle.paymentType == Payment.ERC1400 && erc1400TokenData.length == 0
      ) {
   ...
   * @return Order value.
   */
  function _getOrderAmountAndValue(
    uint256 orderIndex
   ...
   */
  function getTotalSettledForCycle(
    uint256 cycleIndex
   ...
   * @return 'True' if there are remaining orders to settle, 'false' if not.
   */
  function _getTotalSettledForCycle(
    uint256 cycleIndex
  ...
  function getCycleState(
    uint256 cycleIndex
  ) external view returns (CycleState) {
  ...
  function _getCycleState(
    uint256 cycleIndex
  ) internal view returns (CycleState) {
    Cycle storage cycle = _cycles[cycleIndex];
   ...
   */
  function _getTransferFlag(
    bytes memory data
  ) internal pure returns (bytes32 flag) {
    assembly {
  ...
  function _getAssetAddress(
    bytes memory data
  ) internal pure returns (address assetAddress) {
    assembly {
  ...
  function _getAssetClass(
    bytes memory data
  ) internal pure returns (bytes32 assetClass) {
    assembly {
  ...
  function _getOrderData(
    bytes memory data
  ) internal pure returns (bytes memory orderData) {
    uint256 orderValue;
    uint256 orderAmount;
    OrderType orderType;
    assembly {
   ...
   */
  function _getOrderIndex(
    bytes memory data
  ) internal pure returns (uint256 orderIndex) {
    assembly {
   ...
   */
  function setPriceOracles(
    address tokenAddress,
    address[] calldata oracles
  ) external onlyPriceOracle(tokenAddress) {
   ...
   */
  function getAssetValueRules(
    address assetAddress,
    bytes32 assetClass
  ) external view returns (AssetValue, uint256, uint256) {
    AssetRules storage rules = _assetRules[assetAddress][assetClass];
  ...
  function getOrder(
    ...
    external
    view
    returns (uint256, address, uint256, uint256, OrderType, OrderState)
  {
   ...
   * @return Order value.
   */
  function getOrderAmountAndValue(
    uint256 orderIndex
