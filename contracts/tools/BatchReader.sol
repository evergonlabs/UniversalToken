...
abstract contract IERC1400TokensValidatorExtended is IExtensionTypes {
  ...
  function retrieveTokenSetup(
    address token
  )
    external
    view
    virtual
    returns (CertificateValidation, bool, bool, bool, bool, address[] memory);
...
contract BatchReader is IExtensionTypes, ERC1820Client, ERC1820Implementer {
    ...
    )
  {
    ...
    (
      ...
      bytes32[] memory batchDefaultPartitions_
    ...
    )
  {
    (
      ...
      address[] memory batchExtensionControllers_
    ) = batchExtensionControllers(tokens);

    (
      uint256[] memory batchControllersLength,
      address[] memory batchControllers_
   ...
   */
  function batchExtensionControllers(
    address[] memory tokens
  ) public view returns (uint256[] memory, address[] memory) {
   ...
   */
  function batchTokenExtensionSetup2(
    address[] memory tokens
  ...
  function batchBalanceOf(
    ...
    for (uint256 i = 0; i < tokenHolders.length; i++) {
      for (uint256 j = 0; j < tokens.length; j++) {
        batchBalanceOfResponse[i * tokens.length + j] = IERC20(tokens[j])
  ...
  function batchBalanceOfByPartition(
    ...
    uint256[] memory batchBalanceOfByPartitionResponse = new uint256[](
      tokenHolders.length * batchTotalPartitions_.length
    );
      ...
      for (uint256 j = 0; j < tokens.length; j++) {
        for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
          batchBalanceOfByPartitionResponse[
            i * batchTotalPartitions_.length + counter
  ...
  function batchSpendableBalanceOfByPartition(
    ...
    uint256[] memory batchSpendableBalanceOfByPartitionResponse = new uint256[](
      tokenHolders.length * batchTotalPartitions_.length
    );
   ...
   */
  function batchDefaultPartitions(
    address[] memory tokens
  ) public view returns (uint256[] memory, bytes32[] memory) {
      ...
      for (uint256 j = 0; j < tokens.length; j++) {
        ...
        if (tokenExtension != address(0)) {
          batchAllowlistedResponse[
            i * tokens.length + j
          ] = IERC1400TokensValidatorExtended(tokenExtension).isAllowlisted(
            tokens[j],
            tokenHolders[i]
          );
        } else {
      ...
      for (uint256 j = 0; j < tokens.length; j++) {
        ...
        if (tokenExtension != address(0)) {
          batchBlocklistedResponse[
            i * tokens.length + j
          ] = IERC1400TokensValidatorExtended(tokenExtension).isBlocklisted(
            tokens[j],
            tokenHolders[i]
          );
        } else {
