...
contract ERC20HoldableToken is ERC20Token, IERC20HoldableToken {
  ...
  function generateHoldId(
    ...
    holdId = keccak256(
      abi.encodePacked(recipient, notary, amount, expirationDateTime, lockHash)
    );
  }
  ...
  function retrieveHoldData(
    bytes32 holdId
  ) external view override returns (ERC20HoldData memory) {
    return holds[holdId];
  }
  ...
  function executeHold(bytes32 holdId, bytes32 lockPreimage) public override {
    ...
    if (holds[holdId].secretHash != bytes32(0)) {
      require(
        holds[holdId].secretHash == sha256(abi.encodePacked(lockPreimage)),
  ...
  function holdStatus(
    bytes32 holdId
  ) public view override returns (HoldStatusCode) {
