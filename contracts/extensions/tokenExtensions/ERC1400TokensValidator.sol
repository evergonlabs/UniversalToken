  ...
  IHoldableERC1400TokenExtension
{
  ...
  function tokensToValidate(
    ...
    {
      require(
        _canValidateAllowlistAndBlocklistToken(msg.sender, payload, from, to),
        "54"
  ...
  function _canValidateCertificateToken(
    ...
    if (
      ...
      if (_certificateActivated[token] == CertificateValidation.SaltBased) {
        ...
        if (
          _checkNonceBasedCertificate(token, operator, payload, certificate)
        ) {
  ...
  function _canValidateHoldableToken(
    ...
    if (_holdsActivated[token] && from != address(0)) {
      if (operator != from) {
        ...
        if (
          _holdCanBeExecutedAsNotary(hold_, operator, value) &&
          value <= IERC1400(token).balanceOfByPartition(partition, from)
        ) {
          return true;
        }
      }
   ...
   */
  function granularityByPartition(
    address token,
    bytes32 partition
   ...
   */
  function totalSupplyOnHoldByPartition(
    address token,
    bytes32 partition
  ) external view returns (uint256) {
  ...
  function _canHoldOrCanPreHold(
    ...
    (
      ...
      CertificateValidation certificateControl,
      bytes32 salt
    ) = _canValidateCertificateToken(token, msg.data, operator, certificate);
   ...
   */
  function _functionSupportsCertificateValidation(
    bytes memory payload
  ) internal pure returns (bool) {
    bytes4 functionSig = _getFunctionSig(payload);
    if (
      functionSig == ERC20_TRANSFER_ID || functionSig == ERC20_TRANSFERFROM_ID
    ) {
      return false;
    } else {
      return true;
